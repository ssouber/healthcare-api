...
Minio is used as the local S3-compatible storage.

Once the environment is running, you need to create the bucket manually:

1. Go to [http://localhost:8900](http://localhost:8900)
2. Log in with the following credentials:
    - **Username:** `sail`
    - **Password:** `password`
3. Create a bucket and name it as needed
4. In your .env file you should set `FILESYSTEM_DISK=s3`
5. Configure your envs:
    ```
    ...
    ```
6. You need to create an alias in your machine hosts system to translate the minio host
   run
```bash
sudo su
# then you need to set you laptop password
